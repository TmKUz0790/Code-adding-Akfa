...
def send_serial_numbers(request):
    if request.user.email != 'admin_tmk@gmail.com':
        return render(request, 'send_serial_numbers.html', {'error': 'You are not authorized to perform this action.'})
    ...
    if request.method == 'POST':
        ...
        if failed_serials:
            error_message = f"Failed to send the following serial numbers: {', '.join(failed_serials)}"
            return render(request, 'send_serial_numbers.html', {'error': error_message})
        else:
            success_message = "All serial numbers were sent successfully."
